...
def _load_account_count_config() -> dict:
    ...
    defaults["mode"] = os.environ.get("MODE", defaults.get("mode", "default"))
    if not _proxy_runtime_enabled(defaults.get("mode", "default")):
        defaults["proxy"] = ""

    cpa_base_url = str(defaults.get("cpa_base_url", "") or "").strip().rstrip("/")
    if cpa_base_url and not defaults.get("upload_api_url"):
        defaults["upload_api_url"] = cpa_base_url + "/auth-files"
    if defaults.get("cpa_management_key") and not defaults.get("upload_api_token"):
        defaults["upload_api_token"] = defaults["cpa_management_key"]
    return defaults
...
def build_register_input(params: dict, cfg: dict) -> str:
    ...
    """
    lines = []
    proxy_enabled = _proxy_runtime_enabled(cfg.get("mode", "default"))
